...
runInstruction (Lit index) cs@(CS { s = s, cp = cp, ia = ia, m = (si, mem) }) =
  -- get index's value from the const-pool (Int or Null) and push it to the Stack 
  cs { s = s', ia = ia + 1, m = (si + 1, m') }
    where
      object = cp ! index
...
runInstruction (Print format count) cs@(CS { s = s, o = o, ia = ia, m = (_, mem) }) =
...
runInstruction (SetLocal index) cs@(CS { c = c, s = s, ia = ia, m = (_, mem) }) =
...
runInstruction (Branch index) cs@(CS { p = p, s = s, cp = cp, ia = ia, m = (_, mem) }) =
  ...
  cs { s = s', ia = ia' }
    where
      ...
      ia' =
        case val of
          Runtime.Null -> ia + 1
          _ -> p ! (slot2String label cp) -- TODO: maybe check if label is (Slot addr)

runInstruction (Goto index) cs@(CS { p = p, cp = cp, ia = ia, m = (_, mem) }) =
...
runInstruction (Call index count) cs@(CS { c = c, g = g, s = s, cp = cp, ia = ia, m = (_, mem), fns = fns }) =
  ...
  state { ia = ia + 1 }
    where
      ...
      state =
        runProgram' bodyProg $ cs { c = Frame { arguments = ptrs, variables = initVals, caller = ia } : c, s = s', ia = 0 }
